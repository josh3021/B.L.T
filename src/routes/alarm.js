...
module.exports = app => {
    ...
    app.get('/isChildIn', (req, res) => {
        if(!req.session.username === req.query.username)
            res.redirect('/');
        var childX = req.query.childX; //126정도 값
        var childY = req.query.childY; //37정도 값
	    var telephone = req.query.telephone;
	
	    console.log('query: '+req.query.username+', '+childX+', '+childY+', '+req.query.telephone);
        ...
        if(childX > req.session.ne_x || childY > req.session.ne_y || childX < req.session.sw_x || childY < req.session.sw_y) {
	        ...
	        return res.json({message: 'child is outBounds! find UR child Immedietly!'});
        }
